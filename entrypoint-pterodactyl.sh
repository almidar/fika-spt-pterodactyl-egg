...
# Pterodactyl/Pelican Panel compatible entrypoint
# Uses /home/container as working directory
...
enable_spt_listen_on_all_networks=${LISTEN_ALL_NETWORKS:-true}
...
fika_remote_SHA=$(curl -s "https://api.github.com/repos/project-fika/Fika-Server-CSharp/git/refs/tags/v$fika_version" | grep -oP '"sha":\s*"\K[^"]+' || echo "")

auto_update_spt=${AUTO_UPDATE_SPT:-true}
auto_update_fika=${AUTO_UPDATE_FIKA:-true}
...
validate() {
    ...
    if [[ -d $nodejs_spt_data_dir && -f $spt_nodejs_core_config ]]; then
        ...
        if [[ $existing_spt_version != "null" && $existing_spt_version != "$spt_version" ]]; then
            ...
            echo "  === FATAL ERROR ==="
            echo ""
            echo "  The existing server files mounted to /home/container appear to be from SPT Server version $existing_spt_version"
    ...
    if [[ -d $spt_data_dir ]]; then
        ...
        if [[ -f $fika_mod_dir/FikaServer.dll ]]; then
            fika_local_SHA=$(exiftool -s -s -s -ProductVersion $fika_mod_dir/FikaServer.dll | grep -oP '[0-9.]+\+\K.*' || echo "")
        fi
        if [[ "$fika_local_SHA" != "$fika_remote_SHA" && -n "$fika_remote_SHA" ]]; then
...
echo "Starting EFT SPT Server (Pterodactyl Compatible)"
...
if [[ "$enable_profile_backup" == "true" ]]; then
    ...
    echo "  ==============="
    # Note: crond may not work in Pterodactyl, but we log it anyway
fi
